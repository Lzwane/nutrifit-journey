...
type DeviceCategory = "ios" | "chrome" | "samsung_android" | "desktop_other";

export function FullAppInstallGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // 1. If running inside installed standalone PWA app, never show
    const isStandalone =
    ...
    // 2. Check if user already saw this on their browser (persists on refresh)
    ...
    // 3. First time browser visitor -> Display Gate
    setShouldShow(true);

    const ua = window.navigator.userAgent.toLowerCase();
    const isApple = /iphone|ipad|ipod|macintosh/.test(ua) && !ua.includes("chrome");
    const isSamsung = ua.includes("samsungbrowser");
    const isChromeOrChromium = ua.includes("chrome") || ua.includes("crios") || ua.includes("edg");
  ...
  // If already seen, refreshed, or installed, render the full application directly
  if (!shouldShow) {
    return <>{children}</>;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-between bg-slate-950 text-white p-5 sm:p-8 overflow-y-auto font-sans animate-in fade-in duration-200">
      ...
      <div className="w-full max-w-md my-auto text-center space-y-4 z-10 py-4">
        ...
        {/* 3. ANDROID STEPS */}
        {(activeDevice === "samsung_android" || activeDevice === "desktop_other") && (
          <div className="rounded-2xl border border-emerald-500/30 bg-slate-900/80 p-4 text-left text-xs space-y-3 shadow-lg">
            <p className="font-bold text-emerald-400 flex items-center gap-1.5 text-xs uppercase tracking-wider">
